...
yPos += yPosDiff;	
::cellDbgFontPuts(xPos, yPos, nFontSize, 0xffffffff, " gamePKG Tool v1.1 - by CaptainCPS-X Translate in ita by consolehackdev team [2012] [Yes the UI sucks...LOL]");
yPos += yPosDiff;
::cellDbgFontPuts(xPos, yPos, nFontSize, 0xffffffff, "----------------------------------------------------------------------" );
yPos += yPosDiff;
::cellDbgFontPuts(xPos, yPos, nFontSize, 0xffffffff, "PREMERE -(O)- PER USCIRE E RITORNARE ALLA XMB");
yPos += yPosDiff;
::cellDbgFontPuts(xPos, yPos, nFontSize, 0xffffffff, "PREMERE -(X)- METTERE IN CODA I PKG NELLA XMB");
yPos += yPosDiff;
//::cellDbgFontPuts(xPos, yPos, nFontSize, 0xffffffff, "PREMERE -(/\)- VEDERE IL FILE INFO");
//yPos += yPosDiff;
//::cellDbgFontPuts(xPos, yPos, nFontSize, 0xffffffff, "PREMERE -([ ])- PER RIMUOVERE DALLA CODA");
//yPos += yPosDiff;
//::cellDbgFontPuts(xPos, yPos, nFontSize, 0xffffffff, "PREMERE -[SELECT]- PER CANCELLARE TUTTE LE CODE E AGGIORNARE I DISPOSITIVI");
//yPos += yPosDiff;
::cellDbgFontPuts(xPos, yPos, nFontSize, 0xffffffff, "----------------------------------------------------------------------" );
yPos += yPosDiff;
...
{
// already queued...
::cellMsgDialogOpen2(CELL_MSGDIALOG_DIALOG_TYPE_NORMAL"Spiacente, hai già in coda questo PKG.", callbackfunction, NULL, NULL);
} else {
gamePKG->QueuePKG();	
}
}
...
{
char szMsg[256] = "";
sprintf(szMsg, "Processing \"%s\" [%.2f %s]...\n\nAttendere, questo potrebbe richiedere tempo a seconda delle dimensioni del PKG. Non spegnere il sistema.",
gamePKG->szFileIn,
